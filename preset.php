 ...
 * Preset Menu
 *
 * This is the page that is the menu item in the config callforpaper
 * pages.
 *
...
use mod_callforpaper\output\action_bar;
use mod_callforpaper\output\preset_preview;
...
if ($id) {
    ...
    // We must have the callforpaper activity id.
...
$url = new moodle_url('/mod/callforpaper/preset.php', array('d' => $callforpaper->id));
...
$PAGE->requires->js_call_amd('mod_callforpaper/deletepreset', 'init');
...
if ($action === 'export') {
    ...
    $presetname = optional_param('presetname', $callforpaper->name, PARAM_FILE);
...
if ($action == 'importzip') {
    ...
    if ($importer->needs_mapping()) {
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('fieldmappings', 'callforpaper'), 2, 'mb-4');
        echo $renderer->importing_preset($callforpaper, $importer);
        echo $OUTPUT->footer();
        exit(0);
    }
    $importer->import(false);
    core\notification::success(get_string('importsuccess', 'mod_callforpaper'));
    redirect(new moodle_url('/mod/callforpaper/field.php', ['id' => $cm->id]));
    exit(0);
}
...
if ($action === 'preview') {
    ...
    $PAGE->set_url($url);
    $titleparts = [
        get_string('preview', 'callforpaper', $preset->name),
        format_string($cm->name),
        format_string($course->fullname),
    ];
    $PAGE->set_title(implode(moodle_page::TITLE_SEPARATOR, $titleparts));
    ...
    echo $OUTPUT->header();
    $actionbar = new action_bar($callforpaper->id, $url);
...
$actionbar = new \mod_callforpaper\output\action_bar($callforpaper->id, $url);
echo $actionbar->get_presets_action_bar();
$presets = new \mod_callforpaper\output\presets($manager, $presets, new \moodle_url('/mod/callforpaper/field.php'), true);

...
use mod_callforpaper\template;
use moodle_url;
use renderer_base;

/**
 * Renderable class for sticky footer in the view pages of the callforpaper activity.
 *
...
class view_footer extends sticky_footer {
     ...
     * @return array callforpaper context for a mustache template
     */
    ...
    public function get_footer_output(renderer_base $output): string {
        ...
        $callforpaper['pagination'] = $output->paging_bar(
            $this->totalcount,
            $this->currentpage,
            $this->nowperpage,
            $this->baseurl
        ...
        if ($parser->get_template_name() != 'singletemplate' && $parser->has_tag('delcheck') && $canmanageentries) {
            ...
            $callforpaper['selectall'] = $output->render($togglercheckbox);

            $callforpaper['deleteselected'] = html_writer::empty_tag('input', [
        ...
        if (callforpaper_user_can_add_entry($instance, $currentgroup, $groupmode, $context)) {
            $addentrylink = new moodle_url(
                '/mod/callforpaper/edit.php',
                ['id' => $cm->id, 'backto' => $this->baseurl]
            );
            $addentrybutton = new action_link(
                $addentrylink,
                get_string('add', 'mod_callforpaper'),
                null,
                ['class' => 'btn btn-primary mx-1', 'role' => 'button']
            );
            $callforpaper['addentrybutton'] = $addentrybutton->export_for_template($output);
        }
        return $output->render_from_template('mod_callforpaper/view_footer', $callforpaper);
    }
}

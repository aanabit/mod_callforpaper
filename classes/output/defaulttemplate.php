...
/**
 * Renderable class for the default templates in the callforpaper activity.
 *
...
class defaulttemplate implements templatable, renderable {
    ...
    /**
     * Obtains the mustache template name for this callforpaper template.
     *
     * @return string the file mustache path for this template.
     */
    public function get_templatename(): string {
        return 'mod_callforpaper/defaulttemplate_' . $this->templatename;
    }
     ...
     * @return array The callforpaper to display.
     */
    public function export_for_template(\renderer_base $output): array {
        ...
        if (core_tag_tag::is_enabled(manager::PLUGINNAME, 'callforpaper_records')) {
            // Add tags information only if they are enabled.
            if ($this->isform) {
                $tags = callforpaper_generate_tag_form();
            } else {
                $tags = '##tags##';
            }
            $result['tags'] = $tags;

...
/**
 * Renderable class for the presets table in the callforpaper activity.
 *
...
class presets implements templatable, renderable {
     ...
     *
     * @param manager $manager The callforpaper manager
    ...
    private function get_preset_action_menu(renderer_base $output, $preset, ?int $userid): stdClass {
        ...
        $usepreseturl = new moodle_url('/mod/callforpaper/preset.php', [
            'action' => 'usepreset',
            'cmid' => $this->cmid,
        ]);
        $this->add_action_menu($actionmenu, get_string('usepreset', 'mod_callforpaper'), $usepreseturl, [
        ...
        $previewpreseturl = new moodle_url('/mod/callforpaper/preset.php', [
            'fullname' => $preset->get_fullname(),
            'action' => 'preview',
            'id' => $this->cmid,
        ]);
        $this->add_action_menu($actionmenu, get_string('previewaction', 'mod_callforpaper'), $previewpreseturl, [
                'data-action' => 'preview',
            ]
        );
        ...
        if (!$preset->isplugin) {
            // Edit.
            if ($canmanage) {
                $editactionurl = new moodle_url('/mod/callforpaper/preset.php', [
                    'action' => 'edit',
                    'd' => $this->id,
                ]);
            ...
            // Export.
            $exporturl = new moodle_url('/mod/callforpaper/preset.php', [
                'presetname' => $preset->name,
                'action' => 'export',
                'd' => $this->id,
            ]);
            $this->add_action_menu($actionmenu, get_string('export', 'mod_callforpaper'), $exporturl, [
            ...
            if ($canmanage) {
                ...
                $deleteactionurl = new moodle_url('/mod/callforpaper/preset.php', [
                    'action' => 'delete',
                    'd' => $this->id,
                ]);

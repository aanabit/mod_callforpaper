...
/**
 * Search area for mod_callforpaper activity entries.
 *
...
namespace mod_callforpaper\search;
...
class entry extends \core_search\base_mod {
     ...
     */
    protected $entriescallforpaper = array();

    /**
     * Returns recordset containing required callforpaper for indexing callforpaper entries.
     *
    ...
    public function get_document_recordset($modifiedfrom = 0, ?\context $context = null) {
        ...
        list ($contextjoin, $contextparams) = $this->get_context_restriction_sql(
                $context, 'callforpaper', 'd', SQL_PARAMS_NAMED);
        if ($contextjoin === null) {
            return null;
        }
        ...
        $sql = "SELECT dr.*, d.course
                  ...
                  JOIN {callforpaper} d ON d.id = dr.callforpaperid
          $contextjoin
    ...
    public function get_document($entry, $options = array()) {
        try {
            $cm = $this->get_cm('callforpaper', $entry->callforpaperid, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_callforpaper ' . $entry->id . ' document, not all required callforpaper is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_callforpaper' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }

        // Prepare associative array with callforpaper from DB.
        ...
        // All fields should be already returned as plain text by callforpaper_field_base::get_content_value.
    ...
    public function check_access($id) {
        ...
        $numberofentriesindb = $DB->count_records('callforpaper_records', array('callforpaperid' => $entry->callforpaperid));
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/callforpaper/view.php', array( 'd' => $entry->callforpaperid, 'rid' => $entry->id ));
    }

    /**
     * Link to the callforpaper activity.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        $entry = $this->get_entry($doc->get('itemid'));
        return new \moodle_url('/mod/callforpaper/view.php', array('d' => $entry->callforpaperid));
    }
    ...
    /**
     * Add the callforpaper entries attachments.
     *
     * @param \core_search\document $doc
     * @return void
     */
    public function attach_files($doc) {
        ...
        $cm = $this->get_cm('callforpaper', $entry->callforpaperid, $doc->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get all content fields which have files in them.
        $contentssql = "
                SELECT con.*
                  FROM {callforpaper_content} con
                  JOIN {files} fil
        ...
        foreach ($contents as $content) {
            ...
            $fs = get_file_storage();
            $files = $fs->get_area_files($context->id, 'mod_callforpaper', 'content', $content->id, 'filename', false);
    ...
    /**
     * Get callforpaper entry callforpaper
     *
     * @throws \dml_exception
     * @param int $entryid
     * @return stdClass
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriescallforpaper[$entryid])) {
            $this->entriescallforpaper[$entryid] = $DB->get_record('callforpaper_records', array( 'id' => $entryid ), '*', MUST_EXIST);
        }

        return $this->entriescallforpaper[$entryid];
    }
    ...
    protected function get_fields_for_entries($entry) {
        ...
        $sql = "SELECT dc.*, df.name AS fldname,
                  ...
                  FROM {callforpaper_content} dc, {callforpaper_fields} df
        ...
        $callforpaper = $DB->get_record('callforpaper', ['id' => $entry->callforpaperid]);
        ...
        // Filtering out the callforpaper_content records having invalid fieldtypes.
        foreach ($contents as $content) {
        ...
        // Filtering out contents which belong to fields that aren't present in the addtemplate of the callforpaper activity instance.
        foreach ($filteredcontents as $content) {
    ...
    protected function get_field_class_name(string $fieldtype): ?string {
        ...
        $fieldpath = $CFG->dirroot . '/mod/callforpaper/field/' . $fieldtype . '/field.class.php';
        if (!file_exists($fieldpath)) {
            return null;
        }

        require_once($fieldpath);
        return 'callforpaper_field_' . $fieldtype;
    }

    /**
     * Confirms that callforpaper entries support group restrictions.
     *
     * @return bool True
     */

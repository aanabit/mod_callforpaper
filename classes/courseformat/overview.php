...
namespace mod_callforpaper\courseformat;

use cm_info;
use core\url;
use mod_callforpaper\dates;
...
/**
 * Call for paper activity overview integration.
 *
...
class overview extends \core_courseformat\activityoverviewbase {
    ...
    /** @var manager callforpaper activity manager. */
    private $manager;
    ...
    public function __construct(
        ...
        $this->canviewall = has_capability('mod/callforpaper:approve', $cm->context);
    ...
    public function get_due_date_overview(): ?overviewitem {
        ...
        $name = get_string('duedate', 'callforpaper');
    ...
    public function get_actions_overview(): ?overviewitem {
        ...
        if ($this->manager->get_approval_requested()) {
            ...
            if ($toapprove > 0) {
                $name = get_string('approve', 'callforpaper');
            }
        }
        $alertlabel = get_string('numberofentriestoapprove', 'callforpaper', $toapprove);

        $content = new overviewaction(
            url: new url('/mod/callforpaper/view.php', ['id' => $this->cm->id]),
            text: $name,
    ...
    private function get_extra_totalentries_overview(): overviewitem {
        ...
        // Add total entries.
        return new overviewitem(
            name: $this->stringmanager->get_string('totalentries', 'callforpaper'),
            value: $totalentries,
    ...
    private function get_extra_myentries_overview(): ?overviewitem {
        ...
        return new overviewitem(
            name: get_string('myentries', 'callforpaper'),
    ...
    private function get_extra_entries_overview(): overviewitem {
        if ($this->canviewall) {
            $name = get_string('entries', 'callforpaper');
            ...
            $name = get_string('totalentries', 'callforpaper');
            $totalentries = count($allentries);
        }
        return new overviewitem(
            name: $name,
    ...
    private function get_extra_comments_overview(): overviewitem {
        ...
        if (empty($CFG->usecomments) || (empty($this->manager->get_instance()->comments))) {
            return new overviewitem(
                name: get_string('comments', 'callforpaper'),
                value: 0,
                content: '-',
                textalign: text_align::END,
            );

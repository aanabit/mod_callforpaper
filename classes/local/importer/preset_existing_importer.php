...
/**
 * Call for paper preset importer for existing presets
...
class preset_existing_importer extends preset_importer {
    ...
    public function __construct(manager $manager, string $fullname) {
        ...
        if ($userid &&
            ($userid != $USER->id) &&
            !has_capability('mod/callforpaper:manageuserpresets', $context) &&
            !has_capability('mod/callforpaper:viewalluserpresets', $context)
        ) {
        ...
        $filepath = callforpaper_preset_path($course, $userid, $shortname);

...
abstract class entries_importer {
    ...
    public function __construct(string $importfilepath, string $importfilename) {
        ...
        if ($this->importfiletype !== $this->get_import_callforpaper_file_extension() && $this->importfiletype !== 'zip') {
            throw new coding_exception('Only "zip" or "' . $this->get_import_callforpaper_file_extension() . '" files are '
                . 'allowed.');
        }
    }
     ...
     */
    abstract public function get_import_callforpaper_file_extension(): string;
     ...
     */
    public function get_callforpaper_file_content(): false|string {
        ...
        $this->extract_zip();
        $callforpaperfilenames = array_filter($this->packer->list_files($this->importfilepath),
            fn($file) => pathinfo($file->pathname, PATHINFO_EXTENSION) === $this->get_import_callforpaper_file_extension()
                && !str_contains($file->pathname, '/'));
        if (empty($callforpaperfilenames) || count($callforpaperfilenames) > 1) {
            return false;
        }
        return file_get_contents($this->extracteddir . reset($callforpaperfilenames)->pathname);
    }

...
/**
 * Utility class for exporting data from a mod_callforpaper instance.
 *
...
class utils {
    ...
    /**
     * Exports the data of the mod_callforpaper instance to an entries_exporter object which then can export it to a file format.
     *
     ...
     * @return void
     */
    public static function callforpaper_exportdata(int $callforpaperid, array $fields, array $selectedfields, entries_exporter $exporter,
        ...
        $userdetails = $userdetails && has_capability('mod/callforpaper:exportuserinfo', $context);
        ...
        if ($tags) {
            $header[] = get_string('tags', 'callforpaper');
        }
        if ($userdetails) {
        ...
        if ($time) {
            $header[] = get_string('timeadded', 'callforpaper');
            $header[] = get_string('timemodified', 'callforpaper');
        }
        if ($approval) {
            $header[] = get_string('approved', 'callforpaper');
        }
        $exporter->add_row($header);

        $callforpaperrecords = $DB->get_records('callforpaper_records', array('callforpaperid' => $callforpaperid));
        ksort($callforpaperrecords);
        $line = 1;
        foreach ($callforpaperrecords as $record) {
            // Get content indexed by fieldid.
            if ($currentgroup) {
                $select = 'SELECT c.fieldid, c.content, c.content1, c.content2, c.content3, c.content4 FROM {callforpaper_content} c, '
                    . '{callforpaper_records} r WHERE c.recordid = ? AND r.id = c.recordid AND r.groupid = ?';
                $where = array($record->id, $currentgroup);
            } else {
                $select = 'SELECT fieldid, content, content1, content2, content3, content4 FROM {callforpaper_content} WHERE recordid = ?';
            ...
            if ($content = $DB->get_records_sql($select, $where)) {
                foreach ($fields as $field) {
                    ...
                    if (isset($content[$field->field->id])) {
                        ...
                        if (!empty($contents) && $field->file_export_supported() && $includefiles
                            ...
                            // mod_callforpaper instance content. But it's more important to match the name
                ...
                if ($tags) {
                    $itemtags = \core_tag_tag::get_item_tags_array('mod_callforpaper', 'callforpaper_records', $record->id);

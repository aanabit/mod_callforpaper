...
/**
 * Save callforpaper as preset form.
 *
...
class save_as_preset extends dynamic_form {
    ...
    protected function definition() {
        ...
        $this->_form->addElement('checkbox', 'overwrite', '', get_string('overrwritedesc', 'callforpaper'), ['class' => 'hidden']);
    ...
    protected function get_context_for_dynamic_submission(): context {
        ...
        $cm = get_coursemodule_from_instance('callforpaper', $callforpaper->id, $course->id, null, MUST_EXIST);
    ...
    public function validation($formdata, $files): array {
        ...
        if (!empty($formdata['overwrite'])) {
            ...
            if (!$selectedpreset instanceof preset || !$selectedpreset->can_manage()) {
                $errors['name'] = get_string('cannotoverwritepreset', 'callforpaper');
            }
            ...
            foreach ($sitepresets as $preset) {
                if ($formdata['name'] == $preset->name) {
                    if ($preset->can_manage()) {
                        $errors['name'] = get_string('errorpresetexists', 'callforpaper');
                        $usercandelete = true;
                    } else {
                        $errors['name'] = get_string('errorpresetexistsbutnotoverwrite', 'callforpaper');
                    }
                    break;
                }
            }
    ...
    protected function check_access_for_dynamic_submission(): void {
        ...
        if (!has_capability('mod/callforpaper:managetemplates', $this->get_context_for_dynamic_submission())) {
            throw new moodle_exception('saveaspresetmissingcapability', 'callforpaper');
        }
        ...
        if ($action == 'saveaspreset') {
            ...
            $hasfields = $DB->record_exists('callforpaper_fields', ['callforpaperid' => $instanceid]);
    ...
    public function process_dynamic_submission(): array {
        ...
        $result = false;
        $errors = [];
        $callforpaper = $DB->get_record('callforpaper', array('id' => $formdata->d), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $callforpaper->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('callforpaper', $callforpaper->id, $course->id, null, MUST_EXIST);
        ...
        try {
            ...
            if (!empty($formdata->oldpresetname)) {
                // Update the name and the description, to save the new callforpaper.
            ...
            if ($result) {
                ...
                notification::success(get_string('savesuccess', 'mod_callforpaper', (object)['url' => $previewurl->out()]));
            }
    ...
    /**
     * Load in existing callforpaper as form defaults.
     *
     * @return void
     */

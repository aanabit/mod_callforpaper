...
/**
 * Class file field for callforpaper activity
 *
 * @package    callforpaperfield_file
 * @copyright  2005 Martin Dougiamas
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class callforpaper_field_file extends callforpaper_field_base {
    var $type = 'file';
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        // editing an existing callforpaper entry
        if ($formdata) {
        ...
        $html .= '<div class="mod-callforpaper-input">';
    ...
    function get_file($recordid, $content=null) {
        global $DB;
        if (empty($content)) {
            if (!$content = $this->get_callforpaper_content($recordid)) {
                return null;
            }
        }
        $fs = get_file_storage();
    ...
    function display_browse_field($recordid, $template) {
        ...
        return $icon . '&nbsp;<a class="callforpaper-field-link" href="'.$url.'" >' . s($name) . '</a>';
    }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $files = $fs->get_area_files($this->context->id, 'mod_callforpaper', 'content', $content->id, 'itemid, filepath, filename', false);
        ...
        if (count($files) == 0) {
            ...
            if (count($files) > 1) {
                ...
                debugging('more then one file found in mod_callforpaper instance {$this->callforpaper->id} file field (field id: {$this->field->id}) area during update callforpaper record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
            }
        }
    ...
    public function get_field_params(): array {
        ...
        foreach ($filesizes as $value => $name) {
            if (!((isset($this->field->param3) && $value == $this->field->param3))) {
                $callforpaper['filesizes'][] = ['name' => $name, 'value' => $value, 'selected' => 0];
            } else {
                $callforpaper['filesizes'][] = ['name' => $name, 'value' => $value, 'selected' => 1];
            }
        }

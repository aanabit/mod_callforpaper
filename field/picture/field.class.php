...
/**
 * Class picture field for callforpaper activity
 *
 * @package    callforpaperfield_picture
 * @copyright  2005 Martin Dougiamas
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class callforpaper_field_picture extends callforpaper_field_base {
    var $type = 'picture';
    var $previewwidth  = 50;
    ...
    public function get_callforpaper_content_preview(int $recordid): stdClass {
        return (object)[
            ...
            'recordid' => $recordid,
            'content' => 'callforpaperfield_picture/preview',
            'content1' => get_string('sample', 'callforpaperfield_picture'),
            'content2' => null,
            'content3' => null,
            'content4' => null,
        ];
    ...
    function display_add_field($recordid = 0, $formdata = null) {
        ...
        if ($formdata) {
            ...
            if (!$content = $DB->get_record('callforpaper_content', array('fieldid' => $this->field->id, 'recordid' => $recordid))) {
                // Quickly make one now!
                $content = new stdClass();
                $content->fieldid  = $this->field->id;
                $content->recordid = $recordid;
                $id = $DB->insert_record('callforpaper_content', $content);
                $content = $DB->get_record('callforpaper_content', array('id' => $id));
            }
            file_prepare_draft_area($itemid, $this->context->id, 'mod_callforpaper', 'content', $content->id);
            if (!empty($content->content)) {
                if ($file = $fs->get_file($this->context->id, 'mod_callforpaper', 'content', $content->id, '/', $content->content)) {
        ...
        $str .= '<div class="mod-callforpaper-input">';
        ...
        $str .= '<label for="field_' . $this->field->id . '_alttext">' .
                get_string('alttext', 'callforpaper') .
    ...
    public function validate(stdClass $fieldinput): array {
        ...
        foreach ($widthandheightparams as $param) {
            if (!empty($fieldinput->$param) && !is_numeric($fieldinput->$param)) {
                $errors[$param] = get_string('error_invalid' . $param, 'callforpaperfield_picture');
            }
        }
        return $errors;
    }

    // TODO delete this function and instead subclass callforpaper_field_file - see MDL-16493
    ...
    function display_search_field($value = '') {
        return '<label class="accesshide" for="f_' . $this->field->id . '">' . get_string('fieldname', 'callforpaper') . '</label>' .
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($this->preview) {
            $imgurl = $OUTPUT->image_url('sample', 'callforpaperfield_picture');
        ...
        if ($template == 'listtemplate') {
            ...
            $width = '';
            $height = '';
            $url = new moodle_url('/mod/callforpaper/view.php', ['d' => $this->field->callforpaperid, 'rid' => $recordid]);
        } else {
            $filename = $content->content;
            $url = null;
        }
        $imgurl = moodle_url::make_pluginfile_url($this->context->id, 'mod_callforpaper', 'content', $content->id, '/', $filename);
        ...
        return '<a class="callforpaper-field-link" href="' . $url->out() . '">' . $img . '</a>';
    }

    function update_field() {
        global $DB, $OUTPUT;

        // Get the old field callforpaper so that we can check whether the thumbnail dimensions have changed
        $oldfield = $DB->get_record('callforpaper_fields', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('callforpaper_content', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'callforpaper'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_callforpaper', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $content = $DB->get_record('callforpaper_content', array('fieldid' => $this->field->id, 'recordid' => $recordid));
        if (!$content) {
            $content = (object)array('fieldid' => $this->field->id, 'recordid' => $recordid);
            $content->id = $DB->insert_record('callforpaper_content', $content);
        }
        ...
        switch ($names[2]) {
            case 'file':
                $fs = get_file_storage();
                file_save_draft_area_files($value, $this->context->id, 'mod_callforpaper', 'content', $content->id);
                $usercontext = context_user::instance($USER->id);
                $files = $fs->get_area_files(
                    $this->context->id,
                    'mod_callforpaper', 'content',
                    $content->id,
                ...
                if (count($files) == 0) {
                    ...
                    if (count($files) > 1) {
                        // This should not happen with a consistent callforpaper. Inform admins/developers about the inconsistency.
                        debugging('more then one file found in mod_callforpaper instance {$this->callforpaper->id} picture field (field id: {$this->field->id}) area during update callforpaper record {$recordid} (content id: {$content->id})', DEBUG_NORMAL);
                    }

                    if ($file->get_imageinfo() === false) {
                        $url = new moodle_url('/mod/callforpaper/edit.php', array('d' => $this->field->callforpaperid));
                        redirect($url, get_string('invalidfiletype', 'error', $file->get_filename()));
                    }
                    $content->content = $file->get_filename();
                    $this->update_thumbnail($content, $file);
                }
                $DB->update_record('callforpaper_content', $content);
     ...
     *
     * @param stdClass $record the callforpaper content record the file belongs to
    ...
    /**
     * Provides the necessary code for importing a file when importing the content of a mod_callforpaper instance.
     *
     * @param int $contentid the id of the mod_callforpaper content record
    ...
    public function get_field_params(): array {
        ...
        $callforpaper = parent::get_field_params();

        $course = $DB->get_record('course', ['id' => $this->callforpaper->course]);
        ...
        foreach ($filesizes as $value => $name) {
            if (!((isset($this->field->param3) && $value == $this->field->param3))) {
                $callforpaper['options'][] = ['name' => $name, 'value' => $value, 'selected' => 0];
            } else {
                $callforpaper['options'][] = ['name' => $name, 'value' => $value, 'selected' => 1];
            }
        }

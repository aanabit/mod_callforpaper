...
/**
 * Manager tests class for mod_callforpaper.
 *
 * @package    mod_callforpaper
 * @category   test
 * @copyright  2022 Ferran Recio <ferran@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_callforpaper\manager
 */
final class manager_test extends \advanced_testcase {
     ...
     * @covers ::create_from_callforpaper_record
     */
    public function test_create(): void {
        ...
        $callforpaperrecord = (object)[
            'callforpaperid' => $activity->id,
            'id' => 0,
            'userid' => 0,
            'groupid' => 0,
            'timecreated' => 0,
        ...
        $manager = manager::create_from_callforpaper_record($callforpaperrecord);
        $manageractivity = $manager->get_instance();
        $this->assertEquals($activity->id, $manageractivity->id);
        $managercm = $manager->get_coursemodule();
        $this->assertEquals($cm->id, $managercm->id);
    ...
    public function test_set_module_viewed(): void {
        ...
        $moodleurl = new moodle_url('/mod/callforpaper/view.php', ['id' => $cm->id]);
    ...
    public function test_set_template_viewed(): void {
        ...
        $this->assertEquals($context, $event->get_context());
        $moodleurl = new moodle_url('/mod/callforpaper/templates.php', ['d' => $instance->id]);
        $this->assertEquals($moodleurl, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    }
    ...
    public function test_has_records(): void {
        ...
        $course = $this->getDataGenerator()->create_course();
        $callforpaper = $this->getDataGenerator()->create_module(manager::MODULE, ['course' => $course]);
        $manager = manager::create_from_instance($callforpaper);

        // Empty callforpaper should return false.
        ...
        // Call for paper with records should return true.
    ...
    public function test_has_fields(): void {
        ...
        // Call for paper with fields should return true.
    ...
    public function test_get_available_presets(): void {
        ...
        // Check available presets meet the callforpaperpreset plugins when there are no any preset saved by users.
        $callforpaperpresetplugins = core_component::get_plugin_list('callforpaperpreset');
        $manager = manager::create_from_coursemodule($cm);
        $presets = $manager->get_available_presets();
        $this->assertCount(count($callforpaperpresetplugins), $presets);
        ...
        // Check available presets meet the callforpaperpreset plugins + presets saved manually by users.
        $presets = $manager->get_available_presets();
        $this->assertCount(count($callforpaperpresetplugins) + count($savedpresets), $presets);
        // Confirm that, apart from the "Journal" preset, the ones created manually have been also returned.
        $namepresets = array_map(function($preset) {
            return $preset->name;
        }, $presets);
        $this->assertContains('Image gallery', $namepresets);
        ...
        unassign_capability('mod/callforpaper:viewalluserpresets', $teacherrole->id);
        $presets = $manager->get_available_presets();
        $this->assertCount(count($callforpaperpresetplugins), $presets);
        // Confirm that, at least, the "Image gallery" is one of them.
        $namepresets = array_map(function($preset) {
        ...
        $this->assertCount(count($callforpaperpresetplugins) + 1, $presets);
        // Confirm that, at least, the "Image gallery" is one of them.
        $namepresets = array_map(function($preset) {
            return $preset->name;
        }, $presets);
    ...
    public function test_get_available_plugin_presets(): void {
        ...
        // Check available plugin presets meet the callforpaperpreset plugins.
        $callforpaperpresetplugins = core_component::get_plugin_list('callforpaperpreset');
        $manager = manager::create_from_instance($activity);
    ...
    public function test_get_available_saved_presets(): void {
        ...
        // Unassign the mod/callforpaper:viewalluserpresets capability to the teacher role and check that saved presets are not returned.
        $teacherrole = $DB->get_record('role', ['shortname' => 'teacher']);
        unassign_capability('mod/callforpaper:viewalluserpresets', $teacherrole->id);
    ...
    /**
     * Call for paper provider for test_can_view_preset.
     *
     * @return array
     */
    ...
    public function test_can_export_entries(): void {
        ...
        // Add a field.
        /** @var \mod_callforpaper_generator $generator */
        ...
        unassign_capability('mod/callforpaper:exportallentries', $teacherrole->id);
        $result = $manager->can_export_entries();
        $this->assertEquals(true, $result);

        // Teacher without exportallentries and exportentry can't export entries (unless they have created some entries).
        unassign_capability('mod/callforpaper:exportentry', $teacherrole->id);
        $result = $manager->can_export_entries();
    ...
    public function test_reset_all_templates(): void {
        ...
        $record = $DB->get_record('callforpaper', ['id' => $instance->id]);
        foreach (manager::TEMPLATES_LIST as $templatename => $unused) {
    ...
    /**
     * Call for paper provider for test_reset_templatet.
     *
     * @return array
     */
    ...
    public function test_get_all_entries(): void {
        ...
        // Empty callforpaper should return empty array.
        ...
        $field = $generator->create_field($fieldrecord, $callforpaper);
        $generator->create_entry(
            $callforpaper,
            [$field->field->id => 'Example entry'],
        );
    ...
    public function test_get_all_entries_with_groups(): void {
        ...
        $callforpaper = $this->getDataGenerator()->create_module(manager::MODULE, ['course' => $course, 'groupmode' => SEPARATEGROUPS]);
        $manager = manager::create_from_instance($callforpaper);
    ...
    /**
     * Call for paper provider for test_filter_entries_by_user.
     *
     * @return array
     */
    public static function provider_test_filter_entries_by_user(): array {
        return [
            'Empty callforpaper filtered by current user' => [
                'entries' => [],
                'myentries' => [],
                'expected' => 0,
            ],
    ...
    /**
     * Call for paper provider for test_filter_entries_by_approval.
     *
     * @return array
     */
    public static function provider_test_filter_entries_by_approval(): array {
        return [
            'Empty callforpaper filtered by approved' => [
                'entries' => [],
                'approvalfilter' => 1,
                'expected' => 0,
            ],
            'Call for paper with entries filtered by approved' => [
                'entries' => [1, 0, 1, 1],
                'approvalfilter' => 1,
                'expected' => 3,
            ],
            'Call for paper with entries filtered by non approved' => [
                'entries' => [1, 0, 1, 1],
                'approvalfilter' => 0,
                'expected' => 1,
            ],
    ...
    /**
     * Call for paper provider for test get_comments function.
     *
     * @return array
     */
    ...
    public function test_get_comments_with_groups(): void {
        ...
        $callforpaper = $this->getDataGenerator()->create_module(
            manager::MODULE,

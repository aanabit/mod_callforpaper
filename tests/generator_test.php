 ...
 * @coversDefaultClass \mod_callforpaper_generator
 */
final class generator_test extends \advanced_testcase {
    ...
    public function test_generator(): void {
        ...
        $this->assertEquals(0, $DB->count_records('callforpaper'));

        $course = $this->getDataGenerator()->create_course();

        /** @var mod_callforpaper_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_callforpaper');
        $this->assertInstanceOf('mod_callforpaper_generator', $generator);
        $this->assertEquals('callforpaper', $generator->get_modulename());

        $generator->create_instance(['course' => $course->id]);
        $generator->create_instance(['course' => $course->id]);
        $callforpaper = $generator->create_instance(['course' => $course->id]);
        $this->assertEquals(3, $DB->count_records('callforpaper'));

        $cm = get_coursemodule_from_instance('callforpaper', $callforpaper->id);
        $this->assertEquals($callforpaper->id, $cm->instance);
        $this->assertEquals('callforpaper', $cm->modname);
        $this->assertEquals($course->id, $cm->course);

        $context = \context_module::instance($cm->id);
        $this->assertEquals($callforpaper->cmid, $context->instanceid);

        // Test gradebook integration using low level DB access - DO NOT USE IN PLUGIN CODE!
        $callforpaper = $generator->create_instance(['course' => $course->id, 'assessed' => 1, 'scale' => 100]);
        $gitem = $DB->get_record('grade_items', [
            'courseid' => $course->id,
            'itemtype' => 'mod',
            'itemmodule' => 'callforpaper',
            'iteminstance' => $callforpaper->id,
        ]);
    ...
    public function test_create_field(): void {
        ...
        $callforpaper = $generator->create_instance(['course' => $course->id]);
        $this->assertEquals(1, $DB->count_records('callforpaper'));
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            ${$fieldname} = $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_field($record, $callforpaper);

            $this->assertInstanceOf('callforpaper_field_' . $fieldtype, ${$fieldname});
            $count++;
        }

        $this->assertEquals(count($fieldtypes), $DB->count_records('callforpaper_fields', ['callforpaperid' => $callforpaper->id]));
    }
    ...
    public function test_create_entry(): void {
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_field($record, $callforpaper);
            $count++;
        }

        $fields = $DB->get_records('callforpaper_fields', ['callforpaperid' => $callforpaper->id], 'id');
        ...
        $this->setUser($user1);
        $callforpaperrecordid = $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_entry(
            $callforpaper,
            $fieldcontents,
            $groupa->id,
            $tags
        ...
        $this->assertEquals(1, $DB->count_records('callforpaper_records', ['callforpaperid' => $callforpaper->id]));
        $this->assertEquals(count($contents), $DB->count_records('callforpaper_content', ['recordid' => $callforpaperrecordid]));

        $entry = $DB->get_record('callforpaper_records', ['id' => $callforpaperrecordid]);
        $this->assertEquals($entry->groupid, $groupa->id);

        $contents = $DB->get_records('callforpaper_content', ['recordid' => $callforpaperrecordid], 'id');
        ...
        $this->assertEquals(
            ['Cats', 'mice'],
            array_values(\core_tag_tag::get_item_tags_array('mod_callforpaper', 'callforpaper_records', $callforpaperrecordid))
        );
    }
    ...
    /**
     * Call for paper provider for test_create_preset().
     *
     * @return array
     */

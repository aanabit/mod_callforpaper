@mod @mod_callforpaper @javascript @editor_tiny
Feature: Edit existing callforpaper entries
  In order to modify a callforpaper entry
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | callforpaper     | Test callforpaper name | Intro text | C1     | callforpaper1    |
    And the following "mod_callforpaper > fields" exist:
      | callforpaper | type     | name        | required | description |
      | callforpaper1    | text     | headline    | 1        | Headline    |
      | callforpaper1    | textarea | description | 0        | Description |
    And the following "mod_callforpaper > entries" exist:
      | callforpaper | user     | headline   | description   |
      | callforpaper1    | teacher1 | Headline 1 | Original text |
      | callforpaper1    | teacher1 | Headline 2 |               |

  Scenario: Text areas are filled correctly when editing callforpapersets

...
Feature: Control callforpaper activity entry based on read-only dates
  In order to restrict or allow student entries based on specific dates
  As a teacher
  I need to be able to set read-only dates for the callforpaper activity
  ...
  Scenario Outline: Student can add entries only when the current date falls outside the read-only date range
    Given the following "activities" exist:
      ...
      | callforpaper     | C1     | callforpaper Activity 1 | DB1      | <viewfrom>   | <viewto>   |
    And the following "mod_callforpaper > fields" exist:
      | callforpaper | type | name       |
      | DB1      | text | DB Field 1 |
    When I am on the "callforpaper Activity 1" "callforpaper activity" page logged in as student1

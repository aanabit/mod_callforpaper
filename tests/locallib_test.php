...
final class locallib_test extends \advanced_testcase {
     ...
     * @covers ::callforpaper_search_entries
     */
    public function test_callforpaper_search_entries(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $titlefield = $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_field(
            (object) [
                'name' => 'title',
                'type' => 'text',
                'required' => 1
            ...
            $module);
        $captionfield = $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_field(
            (object) [
                'name' => 'caption',
                'type' => 'text',
                'required' => 1
            ...
            $module);
        $this->getDataGenerator()->get_plugin_generator('mod_callforpaper')->create_entry($module, [
        ...
        $cm = get_coursemodule_from_id('callforpaper', $module->cmid);
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0);
        $this->assertCount(2, $records);
        // Search for entries for "caption" we should return only one of them.
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0, 'caption');
        ...
        list($searcharray, $searchtext) = callforpaper_build_search_array($module, false, [], $defaults, $fn, $ln);
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($module, $cm, \context_course::instance($course->id), 'list', 0, $searchtext,
     ...
     * @covers ::callforpaper_search_entries
     */
    public function test_callforpaper_search_entries_with_groups(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($module, $cm, \context_course::instance($course->id), 'list', $group1->id);
        ...
        list($records, $maxcount, $totalcount, $page, $nowperpage, $sort, $mode) =
            callforpaper_search_entries($module, $cm, \context_course::instance($course->id), 'list', $group2->id);
    ...
    private function assert_record_entries_contains($records, $fieldid, $content) {
        ...
        foreach ($records as $record) {
            $fieldscontent = $DB->get_records('callforpaper_content', ['recordid' => $record->id]);

...
/**
 * callforpaper generator class for mod_callforpaper.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_callforpaper_generator extends testing_module_generator {

    /**
     * @var int keep track of how many callforpaper fields have been created.
     */
    protected $callforpaperfieldcount = 0;

    /**
     * @var int keep track of how many callforpaper records have been created.
     */
    protected $callforpaperrecordcount = 0;
    ...
    public function reset() {
        $this->callforpaperfieldcount = 0;
        $this->callforpaperrecordcount = 0;

        parent::reset();
    }

    /**
     * Creates a mod_callforpaper instance
     *
     * @param array $record
     * @param array $options
     * @return StdClass
    ...
    /**
     * Creates a field for a mod_callforpaper instance.
     *
     * @param stdClass $record
     * @param stdClass|null $callforpaper
     * @return callforpaper_field_base
     */
    public function create_field(?stdClass $record = null, $callforpaper = null) {
        $record = (array) $record;

        $this->callforpaperfieldcount++;

        if (!isset($callforpaper->course)) {
            throw new coding_exception('course must be present in phpunit_util::create_field() $callforpaper');
        }

        if (!isset($callforpaper->id)) {
            throw new coding_exception('callforpaperid must be present in phpunit_util::create_field() $callforpaper');
        } else {
        ...
        if (!isset($record['name'])) {
            $record['name'] = "testField - " . $this->callforpaperfieldcount;
        }

        if (!isset($record['description'])) {
            $record['description'] = " This is testField - " . $this->callforpaperfieldcount;
        }
        ...
        $field = callforpaper_get_field($record, $callforpaper);
        $field->insert_field();
    ...
    /**
     * Creates a field for a mod_callforpaper instance.
     * Keep in mind the default callforpaper field params created in create_field() function!
     * ...if you haven't provided your own custom callforpaper field parameters there.
     ...
     *   $contents['callforpaper'] = 'dd-mm-yyyy'
     *   $contents['menu'] = 'value';
     ...
     *
     * @param stdClass $callforpaperrecord from table {callforpaper}
     * @param array $contents
     * @param int $groupid
     * @param array $tags
     * @param array $options
     * @param int $userid if defined, it will be the author of the entry
     * @return int id of the generated record in table {callforpaper_records}
     */
    public function create_entry($callforpaper, array $contents, $groupid = 0, $tags = [], ?array $options = null, int $userid = 0) {
        global $DB, $USER, $CFG;
        ...
        $this->callforpaperrecordcount++;

        $recordid = callforpaper_add_record($callforpaper, $groupid);

        if (isset($options['approved'])) {
            callforpaper_approve_entry($recordid, !empty($options['approved']));
        } else {
            $approved = null;
        }
        ...
        foreach ($fields as $field) {
            ...
            $field = callforpaper_get_field($field, $callforpaper);
            ...
            if ($field->type === 'date') {
                ...
                if (strlen($itemid) && !is_numeric($itemid)) {
                    ...
                    get_file_storage()->create_file_from_string(['component' => 'user', 'filearea' => 'draft',
                        ...
                        'filename' => $filename],
                        file_get_contents($CFG->dirroot.'/mod/callforpaper/field/picture/pix/sample.png'));
                }
        ...
        foreach ($contents as $fieldid => $content) {
            $field = callforpaper_get_field_from_id($fieldid, $callforpaper);

            if (is_array($content) and in_array($field->type, array('date', 'textarea', 'url', 'picture'))) {
        ...
        if (!empty($tags)) {
            ...
            core_tag_tag::set_item_tags('mod_callforpaper', 'callforpaper_records', $recordid,
    ...
    /**
     * Creates a preset from a mod_callforpaper instance.
     *
     * @param stdClass $instance The mod_callforpaper instance.

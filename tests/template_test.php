...
/**
 * Template tests class for mod_callforpaper.
 *
 * @package    mod_callforpaper
 * @category   test
 * @copyright  2022 Ferran Recio <ferran@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_callforpaper\template
 */
final class template_test extends \advanced_testcase {
    ...
    public function test_parse_entries(
        ...
        if ($enablecomments) {
            ...
            $PAGE->set_url('/mod/callforpaper/view.php');
        }

        $course = $this->getDataGenerator()->create_course();
        $params['course'] = $course;
        $activity = $this->getDataGenerator()->create_module('callforpaper', $params);
        $cm = get_coursemodule_from_id('callforpaper', $activity->cmid, 0, false, MUST_EXIST);
    ...
    /**
     * Call for paper provider for test_parse_entries().
     *
    ...
    public static function parse_entries_provider(): array {
        return [
            ...
            'Teacher moreurl tag' => [
                ...
                'expected' => '|Some .*/mod/callforpaper/view.*{cmid}.*rid.*{entryid}.* tag|',
                'rolename' => 'editingteacher',
                'enableexport' => false,
                'approved' => true,
                'enablecomments' => false,
    ...
    /**
     * Create all the necessary data to enable portfolio export in mod_callforpaper
     *
    ...
    protected function enable_portfolio(stdClass $user) {
        ...
        $DB->insert_record('portfolio_log', [
            ...
            'userid' => $user->id,
            'caller_class' => 'callforpaper_portfolio_caller',
            'caller_component' => 'mod_callforpaper',
            'time' => time(),
        ]);
    }

    /**
     * Enable the ratings on the callforpaper entries.
     *
     * @param context_module $context the activity context
     * @param stdClass $activity the activity record
     * @param array $entries callforpaper entries
     * @param stdClass $user the current user record
    ...
    protected function enable_ratings(context_module $context, stdClass $activity, array $entries, stdClass $user) {
        ...
        $ratingoptions = (object)[
            ...
            'userid' => $user->id,
            'returnurl' => $CFG->wwwroot . '/mod/callforpaper/view.php',
    ...
    /**
     * Call for paper provider for test_parse_add_entry().
     *
     * @return array of scenarios
     */

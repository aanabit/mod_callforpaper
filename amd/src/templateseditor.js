 ...
 *
 * @module      mod_callforpaper/templateseditor
 ...
 *
 * @param {Number} instanceId The callforpaper ID
...
const registerResetButton = (mode) => {
    ...
    resetTemplateAction.addEventListener('click', async(event) => {
        ...
        const params = {
            ...
            templatename: await getString(mode, 'mod_callforpaper'),
        };
        deleteCancel(
            getString('resettemplateconfirmtitle', 'mod_callforpaper'),
            Templates.render('mod_callforpaper/template_editor_resetmodal', params),
    ...
    toggleTemplateEditor.addEventListener('click', async(event) => {
        ...
        if (enableTemplateEditor) {
            ...
            saveCancel(
                ...
                getString('enabletemplateeditorcheck', 'mod_callforpaper'),
                getString('editorenable', 'mod_callforpaper'),
                () => {
                    window.location = relativeUrl('/mod/callforpaper/templates.php', {d: instanceId, mode: mode, useeditor: true});
                },
                null,
                {triggerElement: event.target}
            );
        } else {
            window.location = relativeUrl('/mod/callforpaper/templates.php', {d: instanceId, mode: mode, useeditor: false});
        }
    });
};

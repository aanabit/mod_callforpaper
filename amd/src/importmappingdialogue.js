 ...
 *
 * @module      mod_callforpaper/importmappingdialogue
...
// Load global strings.
prefetchStrings('mod_callforpaper', ['mapping:dialogtitle:usepreset']);
...
const showMappingDialogue = (usepreset) => {
    ...
    getMappingInformation(cmId, presetName).then((result) => {
        if (result.data && result.data.needsmapping) {
            buildModal({
                title: getString('mapping:dialogtitle:usepreset', 'mod_callforpaper', result.data.presetname),
                body: Templates.render('mod_callforpaper/fields_mapping_body', result.data),
                footer: Templates.render('mod_callforpaper/fields_mapping_footer', getMappingButtons(cmId, presetName)),
                large: true,
                show: true,
            });
        } else {
            window.location.href = Url.relativeUrl(
                'mod/callforpaper/field.php',
                {
                    id: cmId,
                    mode: 'usepreset',
                    fullname: presetName,
 ...
 *
 * @param {int} cmId The id of the current callforpaper activity.
 * @param {string} presetName The preset name to delete.
 * @return {array} Same callforpaper with buttons.
 */
...
const getMappingInformation = (cmId, presetName) => {
    const request = {
        methodname: 'mod_callforpaper_get_mapping_information',
        args: {
            cmid: cmId,

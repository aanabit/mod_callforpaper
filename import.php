...
$url = new moodle_url('/mod/callforpaper/import.php');
if ($rid !== 0) {
    $url->param('rid', $rid);
}
if ($fielddelimiter !== '') {
...
if ($id) {
    $url->param('id', $id);
    $PAGE->set_url($url);
    $cm     = get_coursemodule_from_id('callforpaper', $id, 0, false, MUST_EXIST);
    ...
    $url->param('d', $d);
    $PAGE->set_url($url);
    $callforpaper  = $DB->get_record('callforpaper', array('id'=>$d), '*', MUST_EXIST);
    $course = $DB->get_record('course', array('id'=>$callforpaper->course), '*', MUST_EXIST);
    $cm     = get_coursemodule_from_instance('callforpaper', $callforpaper->id, $course->id, false, MUST_EXIST);
}

require_login($course, false, $cm);

$context = context_module::instance($cm->id);
require_capability('mod/callforpaper:manageentries', $context);

$form = new mod_callforpaper_import_form(new moodle_url('/mod/callforpaper/import.php'), ['callforpaperid' => $callforpaper->id,
...
$pagename = get_string('uploadrecords', 'callforpaper');
...
echo $OUTPUT->header();
echo $OUTPUT->heading_with_help($pagename, 'uploadrecords', 'mod_callforpaper');
...
if ($formdata = $form->get_data()) {
    ...
    $importer = new \mod_callforpaper\local\importer\csv_entries_importer($uploadedfilepath, $form->get_new_filename('recordsfile'));

    if (!$importer->get_callforpaper_file_content()) {
        echo $OUTPUT->notification(get_string('errorcallforpaperfilenotfound', 'callforpaper'),
            \core\output\notification::NOTIFY_ERROR);
    } else {
        $importer->import_csv($cm, $callforpaper, $formdata->encoding, $formdata->fielddelimiter);
        ...
        if (count($addedrecordsmessages) > 0) {
            echo $OUTPUT->notification(count($addedrecordsmessages) . ' ' . get_string('recordssaved', 'callforpaper'),
                \core\output\notification::NOTIFY_SUCCESS);
        } else {
            echo $OUTPUT->notification(get_string('recordsnotsaved', 'callforpaper'),
                \core\output\notification::NOTIFY_ERROR);
        }
    }
} else {

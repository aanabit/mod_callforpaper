 ...
 * Used when editing a callforpaper template
 */
...
function showHideAdvSearch(checked) {
    ...
    for (var i = 0; i < divs.length; i++) {
        if(divs[i].id.match('callforpaper_adv_form')) {
            if(checked) {
...
M.callforpaper_urlpicker = {};

M.callforpaper_urlpicker.init = function(Y, options) {
    options.formcallback = M.callforpaper_urlpicker.callback;
...
M.callforpaper_urlpicker.callback = function (params) {
